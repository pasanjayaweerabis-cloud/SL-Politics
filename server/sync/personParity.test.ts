...
describe.skipIf(!HAVE_DB)("PersonPage acceptance set: bundle vs database/API parity", () => {
  ...
  const queryable: Queryable = {
    get: (sql, params) => {
      if (!rawDb) throw new Error("parity database not opened");
      return (rawDb.prepare(sql).get(...((params as never[] | undefined) ?? [])) ?? null) as never;
    },
    all: (sql, params) => {
      if (!rawDb) throw new Error("parity database not opened");
      return rawDb.prepare(sql).all(...((params as never[] | undefined) ?? [])) as never;
    },
  };
